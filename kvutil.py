    ...
    for key,value in cmdlineargs.items():
        ...
        if key in optiondict:
            ...
            if value in ('tbl','table','helptbl','fmt'):
                tblfmt = True
            kv_parse_command_line_display( optiondictconfig, tblfmt=tblfmt, debug=False )
            sys.exit()
        elif raise_error:
...
def kv_parse_command_line_display( optiondictconfig, optiondict={}, tblfmt=False, debug=False ):
